...
def test_missing_signal_marks_matched_item_undone():
    ...
    tracking_agent.handle_status_update(C, "heyet raporunu henüz alamadım")

    checklist = state.get_case(C)["checklist"]
    heyet_item = next(c for c in checklist if "heyet" in c["item"].casefold())
    assert heyet_item["done"] is False
    assert any(n["type"] == "reminder" for n in state.get_case(C)["notifications"])


def test_done_signal_marks_matched_item_done():
    _set_evde_bakim_checklist()

    tracking_agent.handle_status_update(C, "TC kimlik belgesini hazırladım")
...
def test_applied_signal_gives_acknowledgement_without_touching_checklist():
    ...
    message = tracking_agent.handle_status_update(C, "başvurdum")

    assert "başvur" in message.casefold()
    assert all(not c["done"] for c in state.get_case(C)["checklist"])


def test_unmatched_message_falls_back_with_pending_hint():
    _set_evde_bakim_checklist()

    message = tracking_agent.handle_status_update(C, "yarın gideceğim demiştim")
...
def test_scoring_prefers_best_overlap_over_first_item():
    ...
    tracking_agent.handle_status_update(C, "sağlık kurulu heyet raporunu henüz alamadım")
...
def test_stopword_alone_does_not_produce_false_match():
    state.set_checklist(C, [{"item": "Gelir beyanı", "done": False}])

    message = tracking_agent.handle_status_update(C, "bu iş için ne yapmam lazım")
...
def test_appointment_date_extraction_dd_mm_yyyy():
    ...
    tracking_agent.handle_status_update(C, f"MHRS randevumu {soon} tarihine aldım")

    appointments = state.get_case(C)["appointments"]
...
def test_appointment_date_extraction_yarin_keyword():
    tracking_agent.handle_status_update(C, "yarın randevum var")
...
def test_appointment_date_extraction_turkish_month_name():
    ...
    tracking_agent.handle_status_update(C, "randevumu 15 Ağustos tarihine aldım")
...
def test_appointment_date_extraction_turkish_month_name_with_year():
    tracking_agent.handle_status_update(C, "randevum 15 ağustos 2026")
...
def test_appointment_date_extraction_turkish_month_rolls_to_next_year_if_past():
    ...
    tracking_agent.handle_status_update(C, "randevum 5 Ocak")
...
def test_appointment_without_date_asks_clarifying_question():
    message = tracking_agent.handle_status_update(C, "Bir randevu aldım")

    assert "tarih" in message.casefold()
    assert state.get_case(C)["appointments"] == []


def test_far_future_appointment_does_not_trigger_immediate_reminder():
    far = (date.today() + timedelta(days=30)).strftime("%d.%m.%Y")

    tracking_agent.handle_status_update(C, f"randevum {far} tarihinde")

    reminder_notifications = [n for n in state.get_case(C)["notifications"] if n["type"] == "reminder"]
